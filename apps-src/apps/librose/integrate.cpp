...
namespace {
	const int title2_size = font::SIZE_DEFAULT;
	...
	, bubble_anims_()
{
	...
	try {
		parsed_items = utils::to_cfgs(src);
	...
	for (it = parsed_items.begin(); it != parsed_items.end(); ++it) {
		std::string name;
		if (utils::is_single_cfg(it->second, &name)) {
...
void tintegrate::animated_undraw(display& disp, texture& canvas, const SDL_Rect& clip_rect)
{
	if (!exist_anim_) {
		return;
	}
...
void tintegrate::animated_draw(display& disp, texture& canvas, const SDL_Rect& clip_rect)
{
	if (!exist_anim_) {
		return;
	}
	for (std::map<int, int>::const_iterator it = anims_.begin(); it != anims_.end(); ++ it) {
		...
		draw_canvas_anim(disp, it->second, canvas, rect, true);
	}
}
