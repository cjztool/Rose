...
void tdistributor::capture_mouse(const bool capture, twidget* widget)
{
	if (capture) {
		VALIDATE(mouse_focus_, null_str);

		// must one be recapture same widget.
		VALIDATE(!widget || widget != mouse_focus_, null_str);
	}
	mouse_captured_ = capture;

	if (capture && widget) {
		VALIDATE(widget != mouse_focus_, null_str);
...
twidget* tdistributor::mouse_focus_widget() const
{
	if (!mouse_captured_) {
		return nullptr;
	}
...
void tdistributor::signal_handler_sdl_mouse_motion(
		bool& handled
		...
		, const tpoint& coordinate2)
{
...
#endif
		posix_print("------signal_handler_sdl_mouse_motion, magic event(%i, %i), mouse_focus_: %p\n", coordinate.x, coordinate.y, mouse_focus_);

		if (mouse_focus_) {
			mouse_leave(coordinate);
		}
		mouse_left_.is_down_ = false;
		leave_window_ = false;
		post_mouse_up();
		return;
	}
	...
	if (leave_window_) {
		...
		posix_print("signal_handler_sdl_mouse_motion, when leave_window_ = true, receive (%i, %i)\n", coordinate.x, coordinate.y);
		...
		mouse_left_.reenter_window(state);
		mouse_middle_.reenter_window(state);
		mouse_right_.reenter_window(state);
		...
		, const tpoint& coordinate2)
{
	if (mouse_captured_) {
		VALIDATE(mouse_focus_, null_str);
		owner_.fire(event, *mouse_focus_, coordinate, coordinate2);
	} else {
		twidget* mouse_over = owner_.find_at(coordinate, true);
		if (mouse_over) {
			owner_.fire(event, *mouse_over, coordinate, coordinate2);
		}
	}
	handled = true;
}
...
void tdistributor::mouse_leave(const tpoint& coordinate)
{
	posix_print("mouse_leave: %s\n", mouse_focus_->id().empty()? "<nil>": mouse_focus_->id().c_str());
    VALIDATE(mouse_focus_, null_str);

	owner_.fire(event::MOUSE_LEAVE, *mouse_focus_, coordinate, null_point);
...
void tdistributor::start_hover_timer(twidget* widget, const tpoint& coordinate)
{
	...
	hover_timer_ = add_timer(
			  50
			, boost::bind(&tdistributor::show_tooltip, this));
		...
		, const tpoint& coordinate)
{
	...
	posix_print("[#%i]signal_handler_sdl_button_down------(%i, %i), mouse_focus_: %p, window_id(%s)\n",
		times, coordinate.x, coordinate.y, distributor_.mouse_focus_,
	...
	if (!distributor_.mouse_focus_) {
		...
		// 3. down immediate! because no moiton, tevent_handler doen't send extra motion, but A's mouse_focus_=nullptr.
		// to solve it, we can send extra motion to A, but it will result to more problem.
		posix_print("distributor_.mouse_focus_ is nullptr, we hope motion conitnue, so do nothing\n");
		return;
	}
	...
	if (!mouse_over) {
		posix_print("------[#%i]signal_handler_sdl_button_down[#%i], mouse_over = nullptr, window_id(%s)\n", times, button_, distributor_.owner_.id().empty()? "nill": distributor_.owner_.id().c_str());
		return;
	}

	if (button_ == SDL_BUTTON_LEFT) {
		VALIDATE(mouse_over == distributor_.mouse_focus_, null_str);
	...
	posix_print("------[#%i]signal_handler_sdl_button_down[#%i], window_id(%s)\n", times, button_, distributor_.owner_.id().empty()? "nill": distributor_.owner_.id().c_str());
}
...
{
	const int times = times_ ++;
	posix_print("[#%i]signal_handler_sdl_button_up[#%i]------is_down_: %s, window_id(%s)\n",
		times, button_,
		is_down_? "true": "false", 
		distributor_.owner_.id().empty()? "nill": distributor_.owner_.id().c_str());
	...
	if (button_ == SDL_BUTTON_LEFT) {
		if (distributor_.mouse_captured_) {
			distributor_.mouse_leave(coordinate);
		...
		if (distributor_.mouse_click_ && distributor_.mouse_click_ == mouse_over) {
			...
			if (distributor_.mouse_focus_) {
				...
				distributor_.mouse_leave(tpoint(x, y));
			}
		}
	...
	posix_print("------[#%i]signal_handler_sdl_button_up[#%i], window_id(%s)\n", times, button_, distributor_.owner_.id().empty()? "nill": distributor_.owner_.id().c_str());
}
...
void tdistributor::tmouse_button::reenter_window(uint32_t state)
{
	if (button_ == SDL_BUTTON_LEFT) {
		posix_print("tmouse_button::reenter_window(state:0x%08x), is_down_: %s\n", state, is_down_? "true": "false");
		if (is_down_) {
			if (!(state & SDL_BUTTON(button_))) {
				posix_print("reenter_window(%i), previous's is_down_ is true, will change to false, state: 0x%08x\n", button_, state);
				is_down_ = false;
			}
		} else {
			if (state & SDL_BUTTON(button_)) {
				posix_print("reenter_window(%i), previous's is_down_ is false, will change to true, state: 08x%08x\n", button_, state);
				is_down_ = true;
			}
		}
	}
...
bool tdistributor::tmouse_button::simulate_up()
{
	VALIDATE(button_ == SDL_BUTTON_LEFT, null_str);

	const bool ret = is_down_;
	is_down_ = false;
	return ret;
}
...
bool tdistributor::simulate_left_up()
{
	return mouse_left_.simulate_up();
}
...
void tdistributor::window_connect_disconnected(bool connect)
{
	...
	if (connect) {
		posix_print("window_connect_disconnected: connect: %s, mouse_focus_: %p, window(%s)\n", connect? "true": "false", mouse_focus_, owner_.id().c_str());
		...
		posix_print("window_connect_disconnected: connect: %s, mouse_focus_: %p, window(%s), do nothing\n", connect? "true": "false", mouse_focus_, owner_.id().c_str());
/*
		// B is connected, this is in A, and send extra motion to A.
		VALIDATE(!mouse_focus_, null_str);
		VALIDATE(!enter_leave_, null_str);

		// 1. mouse on A's [a] widget, so mouse_focus = [a].
		// 2. ([a]'s mouse_focus=nullptr) popup B. mouse motion, and result B closed.
		// 3. down immediate! because no moiton, tevent_handler doen't send extra motion.
		// to solve it, when closed window, send extra motion to A.

		twidget* mouse_over = owner_.find_at(coordinate, true);
		posix_print("window_connect_disconnected: connect: %s, mouse_over: %p\n", connect? "true": "false", mouse_over);
