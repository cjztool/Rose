...
void thandler::handle_mouse_motion(const SDL_MouseMotionEvent& motion)
{
	...
	if (!is_mouse_leave_window_event(motion)) {
		window.signal_handler_sdl_mouse_motion(handled, tpoint(motion.x, motion.y));
	}
	if (!handled) {
		window.distributor().signal_handler_sdl_mouse_motion(handled, tpoint(motion.x, motion.y), tpoint(motion.xrel, motion.yrel));
	}
}
