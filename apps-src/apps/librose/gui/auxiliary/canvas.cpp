...
public:
	...
	void draw(texture& canvas, const int canvas_width, const int canvas_height, const game_logic::map_formula_callable& variables, bool blend_none);
...
void tline::draw(texture& canvas, const int canvas_width, const int canvas_height, const game_logic::map_formula_callable& variables, bool blend_none)
{
...
/*
	int ii = 0;
	// require x2 < canvas_width && y2 < canvas_height.
	VALIDATE(
			  static_cast<int>(x1) < canvas_width
				&& static_cast<int>(x2) <= canvas_width
				&& static_cast<int>(y1) < canvas_height
				&& static_cast<int>(y2) <= canvas_height
...
void trectangle::draw(texture& canvas, const int canvas_width, const int canvas_height, const game_logic::map_formula_callable& variables, bool blend_none)
{
	uint32_t fill_color_argb = fill_color_ != FORMULA_COLOR? fill_color_: tformula<unsigned>(fill_color_str_)(variables);
	if (!border_color_ && fill_color_argb == 0) {
		return;
	}
...
/*
	VALIDATE(
			  static_cast<int>(x) < canvas_width
				&& static_cast<int>(x + w) <= canvas_width
				&& static_cast<int>(y) < canvas_height
				&& static_cast<int>(y + h) <= canvas_height
			, _("Rectangle doesn't fit on canvas."));
*/
	if (x >= canvas_width || (x + w) > canvas_width || y >= canvas_height || y + h > canvas_height) {
		// require use VALIDATE. now for scroll_textbox selection.
		return;
	}

	uint32_t argb = SDL_MapRGBA(&get_neutral_pixel_format(),
		((border_color_ & 0xFF000000) >> 24),
		((border_color_ & 0x00FF0000) >> 16),
		((border_color_ & 0x0000FF00) >> 8),
		((border_color_ & 0x000000FF)));
	SDL_Color color = uint32_to_color(argb);
...
void tcircle::draw(texture& canvas, const int canvas_width, const int canvas_height, const game_logic::map_formula_callable& variables, bool blend_none)
{
	/**
	 * @todo formulas are now recalculated every draw cycle which is a bit
	 * silly unless there has been a resize. So to optimize we should use an
...
void timage::draw(texture& canvas, const int canvas_width, const int canvas_height, const game_logic::map_formula_callable& variables, bool blend_none)
{
	/**
	 * @todo formulas are now recalculated every draw cycle which is a  bit
	 * silly unless there has been a resize. So to optimize we should use an
...
void ttext::draw(texture& canvas, const int canvas_width, const int canvas_height, const game_logic::map_formula_callable& variables, bool blend_none)
{
	int font_size = default_font_size_;
	if (font_size_) {
		if (font_size_ < font_min_relative_size) {
			font_size = font_size_;
		} else {
			font_size = font_size_from_relative(font_size_);
		}
		font_size *= twidget::hdpi_scale;
	}
	...
	if (!share_canvas_integrate) {
		...
		surf = font::get_rendered_text2(text, maximum_width, font_size, uint32_to_color(argb), text_editable);
	} else {
	...
	VALIDATE(static_cast<int>(x) < canvas_width && static_cast<int>(y) < canvas_height
	...
	SDL_Rect dst = ::create_rect(x, y, clip.w, clip.h);
...
void tblit::draw(texture& canvas, const int canvas_width, const int canvas_height, const game_logic::map_formula_callable& variables, bool blend_none)
{
/*
	...
	if (!w) {
		w = hdpi_off_[hdpi_w]? canvas_width: (canvas_width / twidget::hdpi_scale);
	} else {
		w *= hdpi_off_[hdpi_w]? 1: twidget::hdpi_scale;
	}
	if (!h) {
		h = hdpi_off_[hdpi_h]? canvas_height: (canvas_height / twidget::hdpi_scale);
	} else {
		h *= hdpi_off_[hdpi_h]? 1: twidget::hdpi_scale;
	}
*/
	...
	const std::vector<image::tblit>& blits = canvas_widget->get_blits();
	for (std::vector<image::tblit>::const_iterator it = blits.begin(); it != blits.end(); ++ it) {
...
void tanim::draw(texture& canvas, const int canvas_width, const int canvas_height, const game_logic::map_formula_callable& variables, bool blend_none)
{
	draw_canvas_anim(*display::get_singleton(), id_, canvas, ::create_rect(0, 0, canvas_width, canvas_height), true);
}
...
*/
		...
		for (std::vector<tshape_ptr>::iterator itor = shapes_.begin(); itor != shapes_.end(); ++ itor) {
			...
			shape.draw(canvas_, w_, h_, variables_, blend_none);
			blend_none = false;
		}
		...
		for (std::vector<int>::const_iterator itor = post_anims.begin(); itor != post_anims.end(); ++ itor) {
			draw_canvas_anim(disp, *itor, canvas_, ::create_rect(0, 0, w_, h_), true);
		}
		...
		for (std::vector<tshape_ptr>::iterator itor = shapes_.begin(); itor != shapes_.end(); ++itor) {
			...
			(*itor)->draw(canvas_, w_, h_, variables_, false);
		}
		...
		for (std::vector<int>::const_iterator itor = post_anims.begin(); itor != post_anims.end(); ++ itor) {
			draw_canvas_anim(disp, *itor, canvas_, ::create_rect(0, 0, w_, h_), false);
		}
	} 
...
void tcanvas::blit(const tcontrol& widget, texture& surf, SDL_Rect rect, bool force, const std::vector<int>& post_anims)
{
	...
	SDL_Rect clip_rect;
	{
		...
		if (is_empty_rect(clip_rect)) {
			clip_rect.w = w_;
			clip_rect.h = h_;
		}

		SDL_Rect r2;
		SDL_IntersectRect(&clip_rect, &rect, &r2);

		clip_rect = calculate_screen_clip_rect(w_, h_, clip_rect, &r);
		if (clip_rect.w <= 0 || clip_rect.h <= 0) {
			return;
		}
		VALIDATE(r2 == r, null_str);

		SDL_Rect clip_rect2 = ::create_rect(r2.x - rect.x, r2.y - rect.y, r2.w, r2.h);
		VALIDATE(clip_rect2 == clip_rect, null_str);
...
/*
	SDL_RenderCopy(get_renderer(), canvas_.get(), NULL, &rect);
*/
}
