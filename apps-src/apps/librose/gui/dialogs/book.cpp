...
tbook::tbook(display& disp, tmap* map, hero_map& heros, const config& game_config, const std::string& tag)
	: disp_(disp)
	, map_(map)
	, heros_(heros)
...
void tbook::ref_at(twindow& window)
{
	...
	tintegrate integrate(current_topic_->text.parsed_text(), label->get_width(), -1, label->get_text_font_size(), font::NORMAL_COLOR);
...
void tbook::section_2_tv_internal(ttree_view_node& htvroot, const help::section& parent)
{
	...
	for (std::vector<help::section *>::const_iterator it = parent.sections.begin(); it != parent.sections.end(); ++ it) {
		...
		ttoggle_panel* toggle = htvi.label();
