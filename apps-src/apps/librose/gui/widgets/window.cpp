...
{ 
	...
	close();
/*
	bool new_eat_left_up = simulate_left_up();
	if (!eat_left_up) {
		posix_print("twindow::set_retval, retval: %i, status_: %i, new_eat_left_up=%i, eat_left_up=%i, id_: %s, evalue\n", retval, status_, new_eat_left_up, eat_left_up, id().c_str());
		eat_left_up = new_eat_left_up;
	} else {
		posix_print("twindow::set_retval, retval: %i, status_: %i, new_eat_left_up=%i, eat_left_up=%i, id_: %s, don't evalue\n", retval, status_, new_eat_left_up, eat_left_up, id().c_str());
	}
*/
}

bool twindow::simulate_left_up()
{
	return event_distributor_->simulate_left_up();
}
...
{
	if (!scene_ && !invalidate_layout_blocked_) {
		need_layout_ = true;
	}
}
...
void twindow::layout()
{
	...
	if (!explicit_x_) {
		VALIDATE(!explicit_y_, null_str);
	} else {
		VALIDATE(explicit_y_ && automatic_placement_, null_str);
	}

	get_screen_size_variables(variables_);

	// for automatic_placement_ = false, variables_ can use (volatile_width)/(volatile_height) 
	variables_.add("volatile_width", variant(settings::screen_width / twidget::hdpi_scale));
	variables_.add("volatile_height", variant(settings::screen_height / twidget::hdpi_scale));
	...
	if (automatic_placement_) {
		...
		variables_.add("volatile_width", variant(size.x / twidget::hdpi_scale));
		variables_.add("volatile_height", variant(size.y / twidget::hdpi_scale));
	...
	/***** Set the window size *****/
	if (!automatic_placement_) {
		// since below ahs execute fill_placeable_width, here needn't.
		fill_placeable_width(size.x);
	}
	place(origin, size);
		...
		, int y_offset)
{
	...
	canvas(1).blit(*this, frame_buffer, calculate_blitting_rectangle(x_offset - draw_offset_.x, y_offset - draw_offset_.y), get_dirty(), anims);
}
...
bool twindow::click_dismiss(const int x, const int y)
{
	if (does_click_dismiss()) {
		if (x == twidget::npos || !did_click_dismiss_except_ || !did_click_dismiss_except_(x, y)) {
			set_retval(CANCEL);
			return true;
		}
	}
...
void twindow::draw_float_widgets()
{
	...
	for (std::vector<std::unique_ptr<tfloat_widget> >::const_iterator it = float_widgets_.begin(); it != float_widgets_.end(); ++it, at ++) {
		...
		if (item.need_layout || !rect.w || !rect.h) {
			...
			VALIDATE(!SDL_RectEmpty(&clip), null_str);
		...
		SDL_RenderCopy(renderer, canvas.get(), NULL, &rect);
	}
...
void twindow::undraw_float_widgets()
{
	...
	for (std::vector<std::unique_ptr<tfloat_widget> >::const_reverse_iterator it = float_widgets_.rbegin(); it != float_widgets_.rend(); ++it) {
		const tcontrol& widget = *(*it)->widget;
...
void twindow::mouse_capture(const bool capture, twidget* widget)
{
	VALIDATE(event_distributor_, null_str);
	event_distributor_->capture_mouse(capture, widget);
}

twidget* twindow::mouse_focus_widget() const
{
	VALIDATE(event_distributor_, null_str);
	return event_distributor_->mouse_focus_widget();
}
...
void twindow::signal_handler_sdl_video_resize(const event::tevent event, bool& handled, const tpoint& new_size)
{
	...
	if (!preferences::set_resolution(disp, new_size2.x, new_size2.y)) {
...
void twindow::signal_handler_sdl_mouse_motion(bool& handled, const tpoint& coordinate)
{
