...
{ 
	...
	result.x += border_size.x / 2;
	result.y += border_size.y / 2;
	result.w -= border_size.x;
	result.h -= border_size.y;
...
void tcontainer_::fill_placeable_width(const int width)
{
	grid_->fill_placeable_width(width - border_space().x);
}
