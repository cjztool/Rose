...
public:
	...
	void erase_child(int at = npos, bool auto_select = true);
	void erase_children();
	void hide_children();

	void set_child_visible(int at, bool visible, bool auto_select = true);
	bool get_child_visible(int at) const;
	void set_child_visible(const tcontrol& widget, bool visible, bool auto_select = true);
	bool get_child_visible(const tcontrol& widget) const;
	void set_child_label(int at, const std::string& label);
	void set_child_label(tcontrol& widget, const std::string& label);
	...
	void set_did_item_pre_change(const boost::function<bool (treport&, ttoggle_button&, ttoggle_button* previous)>& callback)
	{
		did_item_pre_change_ = callback;
	}
	void set_did_item_changed(const boost::function<void (treport&, ttoggle_button&)>& callback)
	{
		did_item_changed_ = callback;
	}
	void set_did_item_click(const boost::function<void (treport&, tbutton&)>& callback)
	{
		did_item_click_ = callback;
	}
...
private:
	...
	bool mini_content_empty() const override { return false; };
