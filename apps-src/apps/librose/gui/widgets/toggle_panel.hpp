...
public:
	...
	/** Inherited from tselectable_ */
	bool set_value(const bool selected, const bool from_ui = false);
	...
	/** Inherited from tselectable_. */
	void set_did_mouse_enter_leave(const boost::function<void (ttoggle_panel&, const bool)>& callback)
	{
		did_mouse_enter_leave_ = callback; 
	}
	void set_did_state_pre_change(const boost::function<bool (ttoggle_panel&)>& callback)
	{
		did_state_pre_change_ = callback; 
	}
	void set_did_state_changed(const boost::function<void (ttoggle_panel&)>& callback)
	{ 
		did_state_changed_ = callback; 
	}
	void set_did_click(const boost::function<void(ttoggle_panel&, const int)>& callback)
	{
		did_click_ = callback;
	}
	void set_did_double_click(const boost::function<void (ttoggle_panel&)>& callback)
	{ 
...
private:
	...
	boost::function<void(ttoggle_panel&, const int)> did_click_;
