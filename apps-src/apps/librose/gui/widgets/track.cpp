...
void ttrack::create_background_texture(texture& screen, const SDL_Rect& rect)
{
	if (background_tex_) {
		int width, height;
...
void ttrack::impl_draw_background(texture& frame_buffer, int x_offset, int y_offset)
{
	...
	create_background_texture(frame_buffer, rect);
...
texture ttrack::get_canvas_tex()
{
	...
	create_background_texture(result, rect);
	if (did_draw_) {
		did_draw_(*this, rect, true);
	}
