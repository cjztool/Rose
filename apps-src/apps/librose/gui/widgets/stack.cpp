...
void tstack::tgrid2::stacked_insert_child(twidget& widget, int at, uint32_t factor)
{
...
tpoint tstack::tgrid2::calculate_best_size() const
{
	...
	tpoint result(0, 0);
	if (stacked_.mode_ == pip || stacked_.mode_ == radio || stacked_.mode_ == vertical) {
...
void tstack::tgrid2::place(const tpoint& origin, const tpoint& size)
{
	...
	if (stacked_.mode_ == pip || stacked_.mode_ == radio) {
		...
		if (!stacked_.upward_) {
			stacked_.std_top_rect_ = null_rect;
			// once upward_ = false, layer(0) is set_orign/set_visible_area, reset to it.
			tupward_lock lock(stacked_); // in order to fake move()
			stacked_.calculate_reserve();
			const int top_rect_h = stacked_.mode_ == vertical? stacked_.std_top_rect_.h: stacked_.std_top_rect_.w;
			stacked_.move(-1 * (top_rect_h - stacked_.reserve_height_));
		}
	}
}
...
void tstack::finalize(const std::vector<std::pair<tbuilder_grid_const_ptr, unsigned> >& widget_builder)
{
	...
	for (std::vector<std::pair<tbuilder_grid_const_ptr, unsigned> >::const_iterator it = widget_builder.begin(); it != widget_builder.end(); ++ it) {
		...
		grid_.stacked_insert_child(*grid, npos, it->second);
	}
}
