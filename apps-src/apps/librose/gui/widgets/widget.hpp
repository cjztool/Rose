...
public:
	...
	class tsimple_place_lock
	{
	public:
		tsimple_place_lock()
			: original_(simple_place)
		{
			simple_place = true;
		}
		~tsimple_place_lock()
		{
			simple_place = original_;
		}
...
public:
	...
	virtual void fill_placeable_width(const int width) {}
	/**
	 * Places the widget.
	 *
	 ...
	 */
	SDL_Rect calculate_clipping_rectangle(const texture& tex, const int x_offset, const int y_offset);
	 ...
	 * @param y_offset            The y offset in the @p frame_buffer to draw.
	 */
	void draw_foreground(texture& frame_buffer, int x_offset, int y_offset);
	...
	const SDL_Rect& clip_rect() const { return clip_rect_; }
