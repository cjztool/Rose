...
void tgrid::fill_placeable_width(const int width)
{
	for (unsigned row = 0; row < rows_; ++row) {
		if (cols_ == 1) {
			...
			cell.widget_->fill_placeable_width(width - cell_border_space(cell).x);
		} else {
			if (col_width_.size() == 0) {
				// parent is fix_rect.
				calculate_best_size();
			}
			int best_width = 0;
			...
			for (unsigned col = 0; col < cols_; ++col) {
				...
				cell.widget_->fill_placeable_width(placeable_width - cell_border_space(cell).x);
			}
		}
	}
}
...
void tgrid::place(const tpoint& origin, const tpoint& size)
{
	...
	// best size(best_size) available size(size)
