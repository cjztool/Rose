...
treport::treport(bool multi_line, bool toggle, const std::string& unit_definition, const std::string& unit_w, const std::string& unit_h, int gap, bool segment_switch, int fixed_cols)
	...
	, unit_size_(twidget::npos, twidget::npos)
	, gap_(gap)
	, toggle_(true)
	...
	, boddy_(NULL)
{
	...
	variables.add("screen_width", variant(settings::screen_width / twidget::hdpi_scale));
	variables.add("screen_height", variant(settings::screen_height / twidget::hdpi_scale));

	unit_size_.x = unit_size_.y = 0;
	if (unit_w_formula_.has_formula2()) {
		unit_size_.x = unit_w_formula_(variables) * twidget::hdpi_scale;
	}
	if (unit_h_formula_.has_formula2()) {
		unit_size_.y = unit_h_formula_(variables) * twidget::hdpi_scale;
	}
...
void treport::finalize_subclass()
{
	...
	// replace content_grid_ with tgrid2.
	content_grid_ = grid2_= new tgrid2(*this);
...
void treport::insert_reserve_widgets()
{
	if (!multi_line_ && !content_grid_->children_vsize()) {
		...
		grid2_->insert_child(*previous_, twidget::npos);
		grid2_->insert_child(*next_, twidget::npos);
	}
}
...
void treport::erase_child(int at, bool auto_select)
{
	int size = content_grid_->children_vsize();
	...
	if (require_reselect) {
		...
		if (new_at != twidget::npos) {
			select(new_at - front_childs_);
		}
	}
}
...
tpoint treport::tgrid2::calculate_best_size() const
{
	...
	return tpoint(fixed_cols * unit_w + (fixed_cols - 1) * report_.gap_, rows * unit_h + (rows - 1) * report_.gap_);
}
...
void treport::mini_place_content_grid(const tpoint& content_origin, const tpoint& content_size, const tpoint& desire_origin)
{
	VALIDATE(content_size.x > 0 && content_size.y > 0, null_str);
	...
	if (!multi_line_) {
		...
		multiline_replacement_children();
	}
}
...
void treport::tabbar_init(bool toggle, const std::string& definition, bool segment_switch)
{
	...
	// previous arrow
	previous_ = create_blits_button("previous", NULL);
	previous_->set_layout_size(tpoint(unit_size_.x, unit_size_.y));
	image::tblit blit("misc/arrow-left.png", image::UNSCALED, unit_size_.x, unit_size_.y);
	...
	// next arrow
	next_ = create_blits_button("next", NULL);
	next_->set_layout_size(tpoint(unit_size_.x, unit_size_.y));
	blit = image::tblit("misc/arrow-right.png", image::UNSCALED, unit_size_.x, unit_size_.y);
...
void treport::multiline_init(bool toggle, const std::string& definition, int fixed_cols)
{
	...
	if (!unit_size_.x || !unit_size_.y) {
		tresolution_definition_ptr ptr = get_control(type, definition);
		if (!unit_size_.x) {
			unit_size_.x = ptr->min_width;
		}
		if (!unit_size_.y) {
			unit_size_.y = ptr->min_height;
		}
	}
...
tcontrol& treport::create_child(const std::string& id, const std::string& tooltip, void* cookie)
{
	...
	if (toggle_) {
		...
		// widget2->set_did_state_pre_change(boost::bind(&treport::pre_toggle, this, _1));
		...
		widget2->set_callback_state_change(boost::bind(&treport::did_changed, this, _1));
		widget = widget2;
...
void treport::set_child_visible(int at, bool visible, bool auto_select)
{
...
void treport::set_child_visible(const tcontrol& widget, bool visible, bool auto_select)
{
	gui2::tgrid::tchild* children = content_grid()->children();
	...
	for (int n = front_childs_; n < childs; n ++) {
		...
		set_child_visible(n - front_childs_, visible, auto_select);
		return;
	}
...
bool treport::get_child_visible(const tcontrol& widget) const
{
...
void treport::set_child_label(int at, const std::string& label)
{
	gui2::tgrid::tchild* children = content_grid()->children();
	const int childs = content_grid()->children_vsize() - back_childs_;
	VALIDATE(at >= 0 && at < childs, null_str);
	...
	at += front_childs_;
	tgrid::tchild& child = children[at];
	tcontrol& widget = *dynamic_cast<tcontrol*>(child.widget_);
...
void treport::set_child_label(tcontrol& widget, const std::string& label)
{
	if (widget.label() == label) {
		return;
	}
...
void treport::select(int at)
{
	...
	select(*children[front_childs_ + at].widget_);
}
...
void treport::tgrid2::layout_init()
{
	twidget::layout_init();
}
...
void treport::tgrid2::calculate_grid_params(int unit_w, int unit_h, int gap, bool extendable, int fixed_cols)
{
	if (stuff_widget_.empty()) {
		if (extendable) {
			...
			VALIDATE(w_ >= (unsigned)(unit_w + gap), "Width of report must not less than unit_w + gap_w!");
		}
...
void treport::tgrid2::erase_child(int at)
{
	...
	// it must be same-size, to save time, don't calcuate linked_group.
	report_.invalidate_layout(false);
}
