...
public:
	...
	void remove_tooltip();
	bool has_tooltip() const { return tooltip_at_ != twidget::npos && float_widgets_[tooltip_at_]->ref_widget_; }
...
private:
	...
	/** Inherited from tevent_handler. */
	bool click_dismiss(const int x = twidget::npos, const int y = twidget::npos);
...
public:
	...
	void mouse_capture(const bool capture, twidget* widget = NULL);
	twidget* mouse_focus_widget() const;
	...
	void signal_handler_sdl_mouse_motion(bool& handled, const tpoint& coordinate);
private:
