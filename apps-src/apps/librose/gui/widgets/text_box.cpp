...
void ttext_box::update_canvas()
{
	...
	BOOST_FOREACH(tcanvas& tmp, canvas()) {
		...
		tmp.set_variable("text_maximum_height", variant(max_height / twidget::hdpi_scale));
		tmp.set_variable("text_editable", variant(get_text_editable()));

		tmp.set_variable("cursor_offset_x", variant((selection_end_.x >= 0? selection_end_.x: selection_start_.x) / twidget::hdpi_scale));
		tmp.set_variable("cursor_offset_y", variant(cursor_offset_y / twidget::hdpi_scale));
		tmp.set_variable("cursor_height", variant(hide_cursor_? 0: text_height_/ twidget::hdpi_scale));
...
void ttext_box::handle_mouse_selection(const tpoint& coordinate, const bool start_selection)
{
	if (label_.empty()) {
        return;
    }
...
void ttext_box::update_offsets()
{
	...
	// update_canvas().
	BOOST_FOREACH(tcanvas& tmp, canvas()) {
		tmp.set_variable("text_font_height", variant(text_height_ / twidget::hdpi_scale));
	}
...
void ttext_box::signal_handler_mouse_motion(const event::tevent event, bool& handled, const tpoint& coordinate)
{
	...
	} else if (start_selection_ticks_) {
...
void ttext_box::signal_handler_left_button_double_click(const event::tevent event, bool& handled)
{
	return;
