...
/*
class dlg_display: public display
{
public:
	dlg_display();
...
{
	static tmap dummy_map("");

	return new display(null_str, NULL, video, &dummy_map, 0);
}

#define KINGDOM_UNIT_RULES_SIZE		14

display::display(const std::string& tile, base_controller* controller, CVideo& video, const tmap* map, size_t num_reports)
...
void display::create_theme()
{
	...
	main_map_area_ = widget->get_rect();
...
map_location display::minimap_location_on(int x, int y)
{
	...
	gui2::tminimap* minimap = dynamic_cast<gui2::tminimap*>(get_theme_object("_mini_map"));
	if (!minimap) {
		return map_location();
	}
...
void display::widget_set_pip_image(const std::string& id, const std::string& bg, const std::string& fg)
{
	...
	if (widget) {
		...
		generate_pip_blits(blits, w, h, bg, fg);
...
void display::widget_set_image(const std::string& id, const std::string& image)
{
	...
	if (widget) {
		...
		widget->set_blits(image::tblit(image, image::UNSCALED, w, h));
	}
}

void display::widget_set_blit(const std::string& id, const image::tblit& blit)
{
	gui2::tcontrol* widget = dynamic_cast<gui2::tcontrol*>(get_theme_object(id));
	if (widget) {
		int w = widget->fix_width();
		...
		blit2.width = w;
		blit2.height = h;
		widget->set_blits(blit2);
	}
}

void display::widget_set_blits(const std::string& id, const std::vector<image::tblit>& blits)
{
	gui2::tcontrol* widget = dynamic_cast<gui2::tcontrol*>(get_theme_object(id));
	if (widget) {
		int w = widget->fix_width();
...
void display::get_fog_shroud_images(std::vector<image::tblit>& res, const map_location& loc, image::TYPE image_type)
{
	...
	BOOST_FOREACH (std::string& name, names) {
		res.push_back(image::tblit(name, image_type));
	}
}
	...
	if (terrains != NULL) {
		...
		for (std::vector<animated<image::locator> >::const_iterator it =
			...
			if (!use_local_light && !roaded) {
				...
				res.push_back(image::tblit(image, off_map? image::SCALED_TO_HEX: image_type));
			} else if (color_mod.empty()) {
				res.push_back(image::tblit(image, image::SCALED_TO_HEX));
			} else {
...
void display::drawing_buffer_add(const tdrawing_layer layer,
		const map_location& loc, int x, int y, const surface& surf, const int width, const int height,
		const SDL_Rect &clip)
{
	if (!surf) {
		return;
	...
	drawing_buffer.push_back(tblit2(layer, loc, x, y, image::tblit(surf, width, height)));
	drawing_buffer.back().surf().back().clip = clip;
}

image::tblit& display::drawing_buffer_add(const tdrawing_layer layer,
		const map_location& loc, int x, int y, const image::locator& loc2, const image::TYPE loc2_type,
		const SDL_Rect &clip)
{
	if (loc2.is_void()) {
		return null_blit;
	...
	tdrawing_buffer& drawing_buffer = to_canvas_? canvas_drawing_buffer_: drawing_buffer_;
	drawing_buffer.push_back(tblit2(layer, loc, x, y, image::tblit(loc2, loc2_type)));
	drawing_buffer.back().surf().back().clip = clip;
	return drawing_buffer.back().surf().back();
}

image::tblit& display::drawing_buffer_add(const tdrawing_layer layer,
			const map_location& loc, int x, int y, const uint32_t color, const int width, const int height)
{
	tdrawing_buffer& drawing_buffer = to_canvas_? canvas_drawing_buffer_: drawing_buffer_;
	drawing_buffer.push_back(tblit2(layer, loc, x, y, image::tblit(color, width, height)));
	return drawing_buffer.back().surf().back();
}
...
void display::drawing_buffer_commit(texture& screen, const SDL_Rect& clip_rect)
{
/*
...
*/
	...
	// posix_print("drawing_buffer_commit, lists: %u, sort: %u\n", drawing_buffer.size(), ticks1 - start);
...
void display::flip()
{
	...
	if (sunset_delay && ++sunset_timer > sunset_delay) {
		...
		const Uint32 color =  SDL_MapRGBA(&video().getformat(), 0, 0, 0, 255);
		// Adjust the alpha if you want to balance cpu-cost / smooth sunset
/*
		int ii = 0;
		// fill_rect_alpha(r, color, 1);
*/
	}
		...
		size_t font_size, SDL_Color color, double x_in_hex, double y_in_hex)
{
	...
	for (int dy=-1; dy <= 1; ++dy) {
		for (int dx=-1; dx <= 1; ++dx) {
			if (dx!=0 || dy!=0) {
				drawing_buffer_add(layer, loc, x + dx, y + dy, back_surf, 0, 0);
			}
		}
	}
	drawing_buffer_add(layer, loc, x, y, text_surf, 0, 0);
}
		...
		Uint32 blendto, double blend_ratio, double submerged, bool vreverse)
{
	...
	if (submerged > 0.0) {
		...
		image::tblit& material2 = drawing_buffer_add(drawing_layer, loc, x, y, *image.loc, image.loc_type, srcrect);
		...
		// simple blit
		image::tblit& material2 = drawing_buffer_add(drawing_layer, loc, x, y, *image.loc, image.loc_type);
...
void display::set_diagnostic(const std::string& msg)
{
	...
	if(msg != "") {
		...
		flabel.set_font_size(font::SIZE_DEFAULT);
...
void display::draw_border(const map_location& loc, const int xpos, const int ypos)
{
	...
	if(loc.x == -1 && loc.y == -1) { // top left corner
		drawing_buffer_add(LAYER_BORDER, loc, xpos + zoom_/4, ypos,
			border_.corner_image_top_left, image::SCALED_TO_ZOOM);
		...
		drawing_buffer_add(LAYER_BORDER, loc, xpos, ypos,
			border_.corner_image_top_right_even, image::SCALED_TO_ZOOM);
	} else if(loc.x == -1 && loc.y == get_map().h()) { // bottom left corner
		drawing_buffer_add(LAYER_BORDER, loc, xpos + zoom_/4, ypos,
			border_.corner_image_bottom_left, image::SCALED_TO_ZOOM);

	} else if(loc.x == get_map().w() && loc.y == get_map().h()) { // bottom right corner
		// We use the map idea of odd and even, and map coords are internal coords + 1
		drawing_buffer_add(LAYER_BORDER, loc, xpos, ypos,
			border_.corner_image_bottom_right_even, image::SCALED_TO_ZOOM);
		// Now handle the sides:
	} else if(loc.x == -1) { // left side
		drawing_buffer_add(LAYER_BORDER, loc, xpos + zoom_/4, ypos,
			border_.border_image_left, image::SCALED_TO_ZOOM);
	} else if(loc.x == get_map().w()) { // right side
		drawing_buffer_add(LAYER_BORDER, loc, xpos + zoom_/4, ypos,
			border_.border_image_right, image::SCALED_TO_ZOOM);
	} else if(loc.y == -1) { // top side
		// We use the map idea of odd and even, and map coords are internal coords + 1
		drawing_buffer_add(LAYER_BORDER, loc, xpos, ypos + zoom_/2,
			border_.border_image_top_odd, image::SCALED_TO_ZOOM);

	} else if(loc.y == get_map().h()) { // bottom side
		// We use the map idea of odd and even, and map coords are internal coords + 1
		drawing_buffer_add(LAYER_BORDER, loc, xpos, ypos + zoom_/2,
			border_.border_image_bottom_odd, image::SCALED_TO_ZOOM);
	}
}
...
void display::set_theme_report_blits(int num, const std::vector<image::tblit>& blits) const
{
...
{
	gui2::tminimap* widget = dynamic_cast<gui2::tminimap*>(get_theme_object("_mini_map"));
	...
	widget->set_blits(image::tblit(screen, area.w, area.h));
}
...
void display::draw_hex(const map_location& loc) 
{
	...
	if (!shrouded(loc)) {
		...
		get_terrain_images(blits, loc,tod.id, image_type, BACKGROUND);
		drawing_buffer_add(LAYER_TERRAIN_BG, loc, xpos, ypos, blits);

		get_terrain_images(blits, loc,tod.id,image_type, FOREGROUND);
		drawing_buffer_add(LAYER_TERRAIN_FG, loc, xpos, ypos, blits);

		// Draw the grid, if that's been enabled
		if (grid_ && on_map && !off_map_tile) {
			drawing_buffer_add(LAYER_GRID_TOP, loc, xpos, ypos, image::grid_top, image::TOD_COLORED);
			drawing_buffer_add(LAYER_GRID_BOTTOM, loc, xpos, ypos, image::grid_bottom, image::TOD_COLORED);
		}
	...
	if(tod_hex_mask1 != NULL || tod_hex_mask2 != NULL) {
		drawing_buffer_add(LAYER_TERRAIN_FG, loc, xpos, ypos, tod_hex_mask1, 0, 0);
		drawing_buffer_add(LAYER_TERRAIN_FG, loc, xpos, ypos, tod_hex_mask2, 0, 0);
	} else if(!tod_hex_mask.empty()) {
		drawing_buffer_add(LAYER_TERRAIN_FG, loc, xpos, ypos, tod_hex_mask, image::SCALED_TO_HEX);
	}
	...
	if (loc == mouseoverHex_ && (on_map || (in_theme() && get_map().on_board_with_border(loc))) && mouseover_hex_overlay_ != NULL) {
		...
		drawing_buffer_add(LAYER_LINGER_OVERLAY, loc, xpos, ypos, mouseover_hex_overlay_, 0, 0);
	}
	...
	if (on_map && show_hover_over_ && loc == mouseoverHex_) {
		drawing_buffer_add(LAYER_UNIT_MISSILE_DEFAULT,
				loc, xpos, ypos, game_config::terrain::mouseover, image::SCALED_TO_HEX);
	}
	...
	if (on_map) {
		if (draw_coordinates_) {
			...
			drawing_buffer_add(LAYER_FOG_SHROUD, loc, off_x, off_y, bg, 0, 0);
			drawing_buffer_add(LAYER_FOG_SHROUD, loc, off_x, off_y, text, 0, 0);
		}
...
void display::draw_float_anim()
{
	...
	for (std::map<int, animation*>::iterator it = area_anims_.begin(); it != area_anims_.end(); ++ it) {
		...
		anim.redraw(video().getTexture(), rect, true);
	}

	drawing_buffer_commit(video().getTexture(), clip_rect_commit());
}
...
void draw_bar_to_blits(std::vector<image::tblit>& blits, const std::string& image, int x, int y, int size, double filled, const SDL_Color& col, fixed_t alpha, bool vtl)
{
	filled = std::min<double>(std::max<double>(filled, 0.0), 1.0);
	...
	if (vtl) {
		...
		blits.push_back(image::tblit(image, image::UNSCALED, top.w, top.h, top, dst_clip.x, dst_clip.y));
		blits.push_back(image::tblit(image, image::UNSCALED, bot.w, bot.h, bot, dst_clip.x, dst_clip.y + top.h));
		...
		blits.push_back(image::tblit(image, image::UNSCALED, top.w, top.h, top, dst_clip.x, dst_clip.y));
		blits.push_back(image::tblit(image, image::UNSCALED, bot.w, bot.h, bot, dst_clip.x + top.w, dst_clip.y));
	}
	...
	if (unfilled < size && alpha >= ftofxp(0.3)) {
		...
		blits.push_back(image::tblit(color, filled_surf_w, filled_surf_h, dst_clip.x, dst_clip.y));
	}
}
