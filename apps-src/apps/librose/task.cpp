...
bool ttask::tsubtask_replace::handle(display& disp)
{
	...
	for (std::vector<tres>::const_iterator it = replace_res_.begin(); it != replace_res_.end(); ++ it) {
		...
		if (!file_replace_string(full_src, replaces2)) {
			...
			gui2::show_message(disp.video(), null_str, vgettext2("Replace $file content fail!", symbols));
			fok = false;
			break;
		}
	}
...
void ttask::fail_rollback(display& disp)
{
