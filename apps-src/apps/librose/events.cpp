...
struct tevents_context
{
	...
	void add_handler(tevent_handler* ptr);
	void remove_handler(tevent_handler* ptr);

	// pump require enum all handler, so can not use std::stack
	std::vector<tevent_handler*> handlers;
	int focused_handler;
};

void tevents_context::add_handler(tevent_handler* ptr)
{
	handlers.push_back(ptr);
}

void tevents_context::remove_handler(tevent_handler* ptr)
{
...
const int tevent_handler::swipe_wheel_level_gap = 100;
const int tevent_handler::swipe_max_normal_level = 10; // must be 10

// @delta: devided by hdpi_scale.
// return 0, no action.
int tevent_handler::delta_2_level(const int delta, const bool finger)
{
	int ret;
...
tevent_handler::tevent_handler(const bool scene, const bool auto_join) 
	: scene_(scene)
	, has_joined_(false)
	, pinch_distance_(0)
	, mouse_motions_(0)
	...
	, last_pinch_ticks_(0)
	, last_motion_coordinate_(gui2::twidget::npos, gui2::twidget::npos)
{
	if (auto_join) {
...
void tevent_handler::handle_event(const SDL_Event& event)
{
	...
	case SDL_FINGERDOWN:
		...
		posix_print("%i, SDL_FINGERDOWN, (%i, %i)\n", now, x, y);
	...
	case SDL_FINGERMOTION:
		{
			...
			// posix_print("%i, SDL_FINGERMOTION, (%i, %i), delta(%i, %i)\n", now, x, y, dx, dy);
	...
	case SDL_FINGERUP:
		// x = event.tfinger.x * screen_width2;
		// y = event.tfinger.y * screen_height2;
		// posix_print("%i, SDL_FINGERUP, (%i, %i)\n", now, x, y);
	...
	case SDL_MOUSEBUTTONUP:
		...
		posix_print("(%s)SDL_MOUSEBUTTONUP[#%i], (%i, %i)\n", scene_? "scene": "gui2", event.button.button, event.button.x, event.button.y);
		{
			...
			if (!multi_gestures2) {
				...
				posix_print("(%s)SDL_MOUSEBUTTONUP, (%i, %i), multi_gestures2 is true, send multi_gestures_up\n", scene_? "scene": "gui2");
		...
		if (event.button.button == SDL_BUTTON_LEFT) {
			last_motion_coordinate_ = tpoint(gui2::twidget::npos, gui2::twidget::npos);
		}
	...
	case SDL_MOUSEMOTION:
		...
		// posix_print("SDL_MOUSEMOTION, xy(%i, %i), ref(%i, %i)\n", event.motion.x, event.motion.y, event.motion.xrel, event.motion.yrel);
		if (!multi_gestures()) {
...
void tevent_handler::mouse_leave_window()
{
	posix_print("(%s)mouse_leave_window\n", scene_? "scene": "gui2");

	last_motion_coordinate_ = tpoint(gui2::twidget::npos, gui2::twidget::npos);
/*
	SDL_MouseMotionEvent motion;
	motion.x = motion.y = gui2::twidget::npos;
	handle_mouse_motion(motion);
*/
}

void tevent_handler::send_extra_mouse_motion(const int x, const int y)
{
	if (last_motion_coordinate_.x == x && last_motion_coordinate_.y == y) {
		return;
	}
	posix_print("(%s)last_motion(%i, %i) isn't desire coordiante(%i, %i), require send extra mouse motion\n", scene_? "scene": "gui2", last_motion_coordinate_.x, last_motion_coordinate_.y, x, y);

	SDL_MouseMotionEvent motion;
	motion.x = x;
	motion.y = y;
	if (last_motion_coordinate_.x == gui2::twidget::npos && last_motion_coordinate_.y == gui2::twidget::npos) {
		motion.xrel = 0; 
		motion.yrel = 0;
	} else {
...
void dump_events(const std::vector<SDL_Event>& events)
{
	...
	for (std::map<int, int>::const_iterator it = dump.begin(); it != dump.end(); ++ it) {
		...
		ss << "(type: " << std::setbase(16) << it->first << ", count: " << it->second << ")";
	}
	posix_print("%s\n", ss.str().c_str());
}
...
void pump()
{
	...
	if (events.size() > 10) {
		posix_print("------waring!! events.size(): %u, last_event: %x\n", events.size(), events.back().type);
		dump_events(events);
	}
	...
	for (ev_it = events.begin(); ev_it != ev_end; ++ev_it){
		...
		const std::vector<tevent_handler*>& event_handlers = contexts.handlers;
