...
int CFG_SIZE_SMALLEST = 10;
int CFG_SIZE_SMALLER = 12;
int CFG_SIZE_SMALL = 14;
int CFG_SIZE_DEFAULT = 16;
int CFG_SIZE_LARGE = 17;
int CFG_SIZE_LARGER = 18;
int CFG_SIZE_LARGEST = 24;
...
tpoint get_rendered_text_size(const std::string& text, int maximum_width, int font_size, const SDL_Color& color, bool editable)
{
	...
	try {
		...
		VALIDATE(rc.w <= maximum_width, null_str);

		return tpoint(rc.w, rc.h);
	}
	catch (utils::invalid_utf8_exception&) {
		// Invalid UTF-8 string
		return tpoint(0, 0);
