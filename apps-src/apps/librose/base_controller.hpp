...
class base_controller : public tevent_handler
{
public:
	enum QUIT_MODE {QUIT_NORMAL, QUIT_TO_DESKTOP, QUIT_RELOAD_DATA, QUIT_ERROR, MIN_APP_QUIT_MODE};
	enum DIRECTION {UP, DOWN, LEFT, RIGHT, NORTH_EAST, SOUTH_EAST, SOUTH_WEST, NORTH_WEST};

	base_controller(int ticks, const config& top_config, CVideo& video);
	virtual ~base_controller();

	void initialize();
	int main_loop();

	virtual void app_create_display() = 0;
	...
	virtual void app_toggle_report(gui2::twidget& widget) {}
	...
	const config& top_config() const { return top_config_; }
