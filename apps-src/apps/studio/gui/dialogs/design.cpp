...
void tdesign::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	portrait->set_did_draw(boost::bind(&tdesign::did_draw_showcase, this, _1, _2, _3));
	...
	strstr.str("");
	tlabel* label = find_widget<tlabel>(&window, "work_directory", false, true);
...
void tdesign::style(twindow& window)
{
	...
	if ((tstyle)dlg.selected_val() == style_) {
		return;
	}
...
void tdesign::did_draw_showcase(ttrack& widget, const SDL_Rect& widget_rect, const bool bg_drawn)
{
...
void tdesign::execute(twindow& window)
{
	...
	if (style_ == style_transition) {
		...
		if (mid_height) {
			...
			items.push_back(tcrop(::create_rect(corner_width, corner_height, mid_width, mid_height), transiton_prefix + "-middle.png"));
		}
	}
