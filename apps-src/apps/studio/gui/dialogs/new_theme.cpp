...
tnew_theme::tnew_theme(display& disp, const tapp_copier& app, const config& core_cfg, tsave_theme& save_theme, tremove_theme& remove_theme)
	: disp_(disp)
	, app_(app)
	, core_cfg_(core_cfg)
	, save_theme_(save_theme)
	, remove_theme_(remove_theme)
	, current_at_(twidget::npos)
{
...
void tnew_theme::pre_show(CVideo& video, twindow& window)
{
	...
	navigate_->set_did_item_pre_change(boost::bind(&tnew_theme::did_item_pre_change_report, this, _1, _2, _3));
	navigate_->set_did_item_changed(boost::bind(&tnew_theme::did_item_changed_report, this, _1, _2));
	...
	for (std::vector<theme::tfields>::const_iterator it = themes_.begin(); it != themes_.end(); ++ it, at ++) {
		...
		tcontrol& widget = navigate_->create_child(fields.id, null_str, NULL);
		widget.set_cookie(reinterpret_cast<void*>(at));
		navigate_->insert_child(widget);
		widget.set_label(form_tab_label(at));
	}
	if (!themes_.empty()) {
		navigate_->select(0);
	}

	tbutton* button = find_widget<tbutton>(&window, "new", false, true);
	...
	button = find_widget<tbutton>(&window, "erase", false, true);
	connect_signal_mouse_left_click(
		  *button
		, boost::bind(
	...
	connect_signal_mouse_left_click(
			...
			, boost::bind(
				...
				, boost::ref(window)));
	connect_signal_mouse_left_click(
			  find_widget<tbutton>(&window, "close", false)
			, boost::bind(
				&tnew_theme::close
				, this
				, boost::ref(window)
...
void tnew_theme::post_show(twindow& window)
{
}
...
void tnew_theme::reload_theme(twindow& window, const int desire_at, const bool initial)
{
	...
	if (initial) {
		text_box->set_text_changed_callback(boost::bind(&tnew_theme::did_id_changed, this, boost::ref(window), _1));
		text_box->set_placeholder(tintegrate::generate_format("new id", "gray"));
	...
	for (int tpl = 0; tpl < theme::color_tpls; tpl ++) {
		...
		if (initial) {
			text_box->set_text_changed_callback(boost::bind(&tnew_theme::did_color_changed, this, boost::ref(window), _1, tpl, theme::normal));
			text_box->set_placeholder(tintegrate::generate_format("alpha, red, green, blue", "gray"));
		}
		text_box->set_active(desire_at != 0);
		...
		if (initial) {
			text_box->set_text_changed_callback(boost::bind(&tnew_theme::did_color_changed, this, boost::ref(window), _1, tpl, theme::disable));
			text_box->set_placeholder(tintegrate::generate_format("alpha, red, green, blue", "gray"));
		}
		text_box->set_active(desire_at != 0);
		...
		if (initial) {
			text_box->set_text_changed_callback(boost::bind(&tnew_theme::did_color_changed, this, boost::ref(window), _1, tpl, theme::focus));
			text_box->set_placeholder(tintegrate::generate_format("alpha, red, green, blue", "gray"));
		}
		text_box->set_active(desire_at != 0);
	}
	...
	if (initial) {
		text_box->set_text_changed_callback(boost::bind(&tnew_theme::did_color_changed, this, boost::ref(window), _1, theme::default_tpl, theme::item_focus));
		text_box->set_placeholder(tintegrate::generate_format("alpha, red, green, blue", "gray"));
	}
	text_box->set_active(desire_at != 0);
	...
	if (initial) {
		text_box->set_text_changed_callback(boost::bind(&tnew_theme::did_color_changed, this, boost::ref(window), _1, theme::default_tpl, theme::item_highlight));
		text_box->set_placeholder(tintegrate::generate_format("alpha, red, green, blue", "gray"));
	}
	text_box->set_active(desire_at != 0);
	...
	if (initial) {
		text_box->set_text_changed_callback(boost::bind(&tnew_theme::did_color_changed, this, boost::ref(window), _1, theme::default_tpl, theme::menu_focus));
		text_box->set_placeholder(tintegrate::generate_format("alpha, red, green, blue", "gray"));
	}
	text_box->set_active(desire_at != 0);
...
bool tnew_theme::did_item_pre_change_report(treport& report, ttoggle_button& widget, ttoggle_button* previous)
{
	if (!previous) {
		return true;
	}

	const int previous_at = (int)reinterpret_cast<long>(previous->cookie());
	...
	previous->set_label(form_tab_label(previous_at));
	return ok;
}

void tnew_theme::did_item_changed_report(treport& report, ttoggle_button& widget)
{
	const int desire_at = (int)reinterpret_cast<long>(widget.cookie());

	reload_theme(*window_, desire_at, current_at_ == twidget::npos);
...
void tnew_theme::new_theme(twindow& window)
{
	...
	tcontrol& widget = navigate_->create_child(default_id, null_str, NULL);
	widget.set_cookie(reinterpret_cast<void*>(at));
	navigate_->insert_child(widget);
	widget.set_label(form_tab_label(at));
	navigate_->select(at);
...
void tnew_theme::erase_theme(twindow& window)
{
	...
	int res = gui2::show_message(disp_.video(), null_str, msg, gui2::tmessage::yes_no_buttons);
	...
	for (int at = current_at_; at < (int)themes_.size(); at ++) {
		tcontrol* widget = navigate_->get_widget(at + 1);
		widget->set_cookie(reinterpret_cast<void*>(at));
	}
	navigate_->erase_child(current_at_);
...
void tnew_theme::save(twindow& window)
{
	...
	save_theme_.handle(disp_);

	cfg_themes_[current_at_].fields = fields;
	cfg_themes_[current_at_].exist_cfg = true;

	navigate_->get_widget(current_at_)->set_label(form_tab_label(current_at_));
	find_widget<tbutton>(&window, "save", false).set_active(false);
}
...
void tnew_theme::close(twindow& window, bool& handled, bool& halt)
{
	...
	if (!same) {
		...
		int res = gui2::show_message(disp_.video(), _("Close"), msg, gui2::tmessage::yes_no_buttons);
		if (res != gui2::twindow::OK) {
			handled = true;
			halt = true;
			return;
