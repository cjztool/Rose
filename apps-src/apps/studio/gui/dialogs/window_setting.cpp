...
void twindow_setting::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	bar_->set_did_item_pre_change(boost::bind(&twindow_setting::did_item_pre_change_report, this, _1, _2, _3));
	bar_->set_did_item_changed(boost::bind(&twindow_setting::did_item_changed_report, this, _1, _2));
...
void twindow_setting::pre_base(twindow& window)
{
	...
	toggle->set_callback_state_change(boost::bind(&twindow_setting::did_theme_toggled, this, _1));
...
void twindow_setting::pre_linked_group(twindow& window)
{
	...
	report.set_did_item_pre_change(boost::bind(&twindow_setting::did_item_pre_change_report, this, _1, _2, _3));
	report.set_did_item_changed(boost::bind(&twindow_setting::did_item_changed_report, this, _1, _2));
	...
	tcontrol& widget2 = report.create_child(null_str, null_str, NULL);
	widget2.set_canvas_variable("image", variant("buttons/cut.png"));
	widget2.set_label("buttons/cut.png");
	report.insert_child(widget2);

	tcontrol& widget3 = report.create_child(null_str, null_str, NULL);
	widget3.set_canvas_variable("image", variant("buttons/erase.png"));
	widget3.set_label("buttons/erase.png");
	report.insert_child(widget3);

	tcontrol& widget4 = report.create_child(null_str, null_str, NULL);
	widget4.set_canvas_variable("image", variant("buttons/erase_child.png"));
	widget4.set_label("buttons/erase_child.png");
	report.insert_child(widget4);

	tcontrol& widget5 = report.create_child(null_str, null_str, NULL);
	widget5.set_canvas_variable("image", variant("buttons/grid.png"));
	widget5.set_label("buttons/grid.png");
	report.insert_child(widget5);
}
...
void twindow_setting::pre_context_menu(twindow& window)
{
	...
	list.set_did_row_changed(boost::bind(&twindow_setting::item_selected, this, boost::ref(window), _1));

	submenu_navigate_ = find_widget<treport>(&window, "submenu_navigate", false, true);
	submenu_navigate_->set_did_item_pre_change(boost::bind(&twindow_setting::did_item_pre_change_report, this, _1, _2, _3));
	submenu_navigate_->set_did_item_changed(boost::bind(&twindow_setting::did_item_changed_report, this, _1, _2));
...
void twindow_setting::pre_float_widget(twindow& window)
{
	...
	patch_bar_->set_did_item_pre_change(boost::bind(&twindow_setting::did_item_pre_change_report, this, _1, _2, _3));
	patch_bar_->set_did_item_changed(boost::bind(&twindow_setting::did_item_changed_report, this, _1, _2));
	...
	tscroll_text_box* text_box = find_widget<tscroll_text_box>(float_widget_page, "float_widget_x_formula", false, true);
	text_box->set_placeholder(tintegrate::generate_format(_("Usable variable: ref_width, ref_height, width, height"), "gray"));

	text_box = find_widget<tscroll_text_box>(float_widget_page, "float_widget_y_formula", false, true);
	text_box->set_placeholder(tintegrate::generate_format(_("Usable variable: ref_width, ref_height, width, height"), "gray"));

	if (!float_widgets.empty()) {
		float_widget_at_ = 0;
		patch_bar_->select(float_widget_at_);
...
bool twindow_setting::save_context_menu(twindow& window)
{
	...
	if (!utils::isvalid_id(report_id, false, min_id_len, max_id_len)) {
		show_id_error(disp_, "report_id", utils::errstr);
		return false;
	}
...
bool twindow_setting::save_float_widget(twindow& window, const int at)
{
	...
	item.ref = text_box->get_value();
	utils::lowercase2(item.ref);
	if (!utils::isvalid_id(item.ref, false, min_id_len, max_id_len)) {
		show_id_error(disp_, "ref", utils::errstr);
		return false;
	}

	// first x
	tscroll_text_box* scroll_text_box = find_widget<tscroll_text_box>(float_widget_page, "float_widget_x_formula", false, true);
	item.x[0].first = scroll_text_box->label();
	utils::lowercase2(item.x[0].first);
	if (!verify_formula(disp_, "x's fomula", item.x[0].first)) {
		return false;
	}

	text_box = find_widget<ttext_box>(float_widget_page, "float_widget_x_offset", false, true);
	if (!utils::isinteger(text_box->label().c_str())) {
		show_id_error(disp_, "x's offset", utils::errstr);
		return false;
	}
	item.x[0].second = atoi(text_box->label().c_str());

	// first y
	scroll_text_box = find_widget<tscroll_text_box>(float_widget_page, "float_widget_y_formula", false, true);
	item.y[0].first = scroll_text_box->label();
	utils::lowercase2(item.y[0].first);
	if (!verify_formula(disp_, "y's fomula", item.y[0].first)) {
		return false;
	}

	text_box = find_widget<ttext_box>(float_widget_page, "float_widget_y_offset", false, true);
	if (!utils::isinteger(text_box->label().c_str())) {
		show_id_error(disp_, "y's offset", utils::errstr);
		return false;
	}
...
bool twindow_setting::did_item_pre_change_report(treport& report, ttoggle_button& widget, ttoggle_button* previous)
{
	if (!previous) {
		return true;
	}
...
void twindow_setting::set_tile_shape(twindow& window)
{
	...
	const config& core_config = controller_.top_config();
...
void twindow_setting::set_app(twindow& window)
{
	...
	cell_.window.app = extract_app_from_textdomain(textdomains_[index]);
...
{
	VALIDATE(float_widget_at_ != twidget::npos && float_widget_at_ < (int)controller_.float_widgets().size(), null_str);
	const ::tfloat_widget& item = controller_.float_widgets()[float_widget_at_];

	tgrid* float_widget_page = bar_panel_->layer(FLOAT_WIDGET_PAGE);

	ttext_box* text_box = find_widget<ttext_box>(float_widget_page, "float_widget_ref", false, true);
	text_box->set_value(item.ref);

	// first x
	tscroll_text_box* scroll_text_box = find_widget<tscroll_text_box>(float_widget_page, "float_widget_x_formula", false, true);
	scroll_text_box->set_label(item.x[0].first);
	text_box = find_widget<ttext_box>(float_widget_page, "float_widget_x_offset", false, true);
	text_box->set_maximum_length(5);
	text_box->set_value(str_cast(item.x[0].second));

	// first y
	scroll_text_box = find_widget<tscroll_text_box>(float_widget_page, "float_widget_y_formula", false, true);
	scroll_text_box->set_label(item.y[0].first);
	text_box = find_widget<ttext_box>(float_widget_page, "float_widget_y_offset", false, true);
	text_box->set_maximum_length(5);
	text_box->set_value(str_cast(item.y[0].second));
}
...
void twindow_setting::append_float_widget(twindow& window)
{
	...
	gui2::tedit_box::tparam param(_("New float widget"), null_str, _("ID"), "_untitled");
	{
		param.text_changed = boost::bind(&mkwin_controller::verify_new_float_widget, &controller_, _1, twidget::npos);
	...
	for (int index = original_size; it != float_widgets.end(); ++ it) {
		tcontrol& widget = navigate.create_child(param.result, null_str, NULL);
...
{
	int res = gui2::show_message(disp_.video(), "", _("Do you want to erase this float widget?"), gui2::tmessage::yes_no_buttons);
	if (res != gui2::twindow::OK) {
		return;
	}
	...
	for (int at = float_widget_at_; at < (int)float_widgets.size(); at ++) {
		const tgrid::tchild& child = navigate.get_child(at + 1);
		child.widget_->set_cookie(reinterpret_cast<void*>(at));
	}
	navigate.erase_child(float_widget_at_);
...
void twindow_setting::rename_float_widget(twindow& window)
{
	...
	gui2::tedit_box::tparam param(_("Rename float widget"), ss.str(), _("ID"), item.u->cell().id);
	{
		param.text_changed = boost::bind(&mkwin_controller::verify_new_float_widget, &controller_, _1, float_widget_at_);
		gui2::tedit_box dlg(disp_, param);
		try {
			dlg.show(disp_.video());
		} catch(twml_exception& e) {
...
{
	if (float_widget_at_ == twidget::npos) {
		return;
	}
/*
	{
		patch_bar_->set_child_visible(float_widget_at_ + 1, true);
		int ii = 0;
		return;
	}
*/
	...
	for (gui2::tgui_definition::tcontrol_definition_map::const_iterator it = controls.begin(); it != controls.end(); ++ it, at ++) {
		...
		ss.str("");
		ss << type;
		items.push_back(gui2::tval_str(at, ss.str()));
		if (type == item.u->widget().first) {
			initial_val = std::distance(controls.begin(), it);
		}
	}
	...
	int selected_val;
	{
		...
		dlg.show(disp_.video());
		selected_val = dlg.selected_val();
		if (selected_val == initial_val) {
			return;
		}
	}
	...
	gui2::tcontrol_definition_ptr ptr = mkwin_display::find_widget(disp_, it->first, "default", null_str);
...
/*
	{
		patch_bar_->set_child_visible(float_widget_at_, false);
		int ii = 0;
		return;
	}
*/
	...
	gui2::tcontrol_setting dlg(disp_, controller_, *item.u.get(), textdomains_, controller_.linked_groups(), true);
	dlg.show(disp_.video());
	int res = dlg.get_retval();
	if (res != gui2::twindow::OK) {
		return;
...
{
	float_widget_at_ = (int)reinterpret_cast<long>(widget->cookie());
...
void twindow_setting::reload_float_widget(treport& navigate, twindow& window)
{
	...
	std::stringstream ss;
	int index = 0;
	for (std::vector<::tfloat_widget>::const_iterator it = float_widgets.begin(); it != float_widgets.end(); ++ it) {
		tcontrol& widget = navigate.create_child(it->u->cell().id, null_str, NULL);
		widget.set_label(form_tab_label(navigate, index));
		widget.set_cookie(reinterpret_cast<void*>(index ++));
		navigate.insert_child(widget);
	}
