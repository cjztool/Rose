...
void tmkwin_scene::app_pre_theme_show()
{
	...
	hotkey::insert_hotkey(HOTKEY_PREVIEW, "preview", _("Preview"));
	...
	// widget->set_blits(image::tblit("buttons/select.png", image::UNSCALED, widget->fix_width(), widget->fix_height()));
	click_generic_handler(*widget, null_str);

	widget = dynamic_cast<tbutton*>(get_object("status"));
	// widget->set_blits(image::tblit("buttons/status.png", image::UNSCALED, widget->fix_width(), widget->fix_height()));
	click_generic_handler(*widget, null_str);

	widget = dynamic_cast<tbutton*>(get_object("grid"));
	// widget->set_blits(image::tblit("buttons/grid.png", image::UNSCALED, widget->fix_width(), widget->fix_height()));
	click_generic_handler(*widget, null_str);
	...
	list->set_did_row_changed(boost::bind(&tmkwin_scene::object_selected, this, boost::ref(*window_), _1));
...
void tmkwin_scene::first_layout(twindow& window)
{
	tbutton* widget = dynamic_cast<tbutton*>(get_object("select"));
	widget->set_blits(image::tblit("buttons/select.png", image::UNSCALED, widget->get_width(), widget->get_height()));

	widget = dynamic_cast<tbutton*>(get_object("status"));
	widget->set_blits(image::tblit("buttons/status.png", image::UNSCALED, widget->get_width(), widget->get_height()));

	widget = dynamic_cast<tbutton*>(get_object("grid"));
	widget->set_blits(image::tblit("buttons/grid.png", image::UNSCALED, widget->get_width(), widget->get_height()));
